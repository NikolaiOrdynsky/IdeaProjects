...
(Спецификации, сопровождающие эти объявления, были адаптированы к заданному интерфейсу, но они не содержат никаких дополнительных условий.)
...
если значение объекта изменяется таким образом, что влияет на сравнение равных, в то время как объект является элементом набора.
...
Попытка добавить недопустимый элемент вызывает непроверенное исключение, обычно исключение NullPointerException или исключение ClassCastException.
Попытка запросить наличие недопустимого элемента может вызвать исключение или просто вернуть значение false;
некоторые реализации будут демонстрировать первое поведение, а некоторые-второе. В более общем плане, попытка выполнить операцию с недопустимым элементом,
завершение которой не приведет к вставке недопустимого элемента в набор, может вызвать исключение или может быть выполнена успешно, по выбору реализации.
